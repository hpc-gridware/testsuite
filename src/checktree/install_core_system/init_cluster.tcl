...
#****** init_cluster/install_spool_database() *********************************
#  NAME
#     install_spool_database() -- create this cluster's PostgreSQL spool database
#
#  SYNOPSIS
#     install_spool_database {}
#
#  FUNCTION
#     Runs before the sge_qmaster installation. inst_qmaster.sh requires the
#     spooling database and the role to exist already - it only runs
#     "spoolinit init" to create the config and jobs tables inside them - so
#     the testsuite has to create both itself.
#
#     The work is done by spool_database_init() in tcl_files/sge_spooling.tcl,
#     which drops and re-creates spool_<commd_port> and the role of the same
#     name. Dropping first is what gives every run a clean database, the way
#     deleting $SGE_ROOT/$SGE_CELL does for classic and berkeleydb spooling.
#
#     A no-op for every other spooling method.
#*******************************************************************************
proc install_spool_database {} {
   ...
   # spool_database_init() logs the reason when it fails
   spool_database_init
}
