...
#*******************************
proc delete_file_at_startup {filename {del_file_name ""}} {
   get_current_cluster_config_array ts_config
...
#****** file_procedures/delete_temp_script_files() ***************************************
#  NAME
...
#     file returned by get_testsuite_delete_filename(). The files are also
#     removed from the internal script cache used in open_remote_spawn_process()
#
#  INPUTS
...
#
#*******************************************************************************
proc delete_temp_script_files {{config_file ""}} {
   global ts_config CHECK_USER

   ts_log_fine "deleting temporary used files ..."

   set local_host [gethostname]
   set del_script "$ts_config(testsuite_root_dir)/scripts/delete_temp_files.sh"

   # first delete local files on the hosts
   set local_del_file_name [get_testsuite_delete_filename 1 $config_file]
   if {[is_remote_file $local_host $CHECK_USER $local_del_file_name]} {
      get_file_content $local_host $CHECK_USER $local_del_file_name local_del_files
      set hosts {}
      for {set i 1} {$i <= $local_del_files(0)} {incr i 1} {
         set help_str [split $local_del_files($i) ":"]
         set del_host [string trim [lindex $help_str 0]]
         if {[lsearch -exact $hosts $del_host] < 0} {
            lappend hosts $del_host
            set num_files($del_host) 1
         } else {
            incr num_files($del_host)
         }
      }
      foreach host $hosts {
         ts_log_fine "deleting $num_files($host) temp files on host $host"
         set result [start_remote_prog $host $CHECK_USER $del_script "$local_del_file_name $host" prg_exit_state 120]
         ts_log_fine $result
      }
      delete_remote_file $local_host $CHECK_USER $local_del_file_name
   }

   # delete temp files in the results directory
   set del_file_name [get_testsuite_delete_filename 0 $config_file]
   set temp_del_file_name "${del_file_name}.old"

   # try to delete the temp files on the fileserver
   set host [fs_config_get_server_for_path $del_file_name 0]
   ...
   if {[file isfile $temp_del_file_name]} {
      ...
      # append files from old tmp file to the actual del file
      get_file_content $local_host $CHECK_USER $temp_del_file_name lost_files
      for {set i 1} {$i <= $lost_files(0)} {incr i} {
         if {[file isfile $lost_files($i)]} {
            ts_log_fine "append file \"$lost_files($i)\" to delete file!"
            delete_file_at_startup $lost_files($i) $del_file_name
         }
      }
      file delete $temp_del_file_name
   }

   # Move the del_file_name file at the local host to <filename>.old
   if {[file isfile $del_file_name]} {
      file rename -force $del_file_name $temp_del_file_name
   } else {
      ts_log_fine "no generated files to delete"
      return
   }

   if {[file isfile $del_file_name]} {
      ts_log_severe "$del_file_name should not be available anymore"
      return
   }

   # Check that renamed file is available on NFS server
   # This call makes 2 new entries in new del_file_name
   if {[wait_for_remote_file $host $CHECK_USER $temp_del_file_name] != 0} {
      ts_log_severe "waiting for file $temp_del_file_name on host \"$host\" failed!"
      return
   }

   get_file_content $local_host $CHECK_USER $temp_del_file_name del_files
   for {set i 1} {$i <= $del_files(0)} {incr i} {
      remove_from_remote_spawn_script_cache $del_files($i)
   }
   ...
   if { $local_host != $host } {
      ...
      # First check that the remote file and local file have the same number of lines
      if {$del_files_remote(0) != $del_files(0)} {
      ...
      for { set i 1} { $i <= $del_files(0) } { incr i 1 } {
         if {$del_files($i) != $del_files_remote($i)} {
            ts_log_severe "local file entry \"$del_files($i)\" does not match remote file entry \"$del_files_remote($i)\""
            return
         }
      }
   }

   # This call makes new entry in new del_file_name and deletes
   # the renamed file on the file server
   ts_log_fine "deleting $del_files(0) files on host $host"
   set result [start_remote_prog $host $CHECK_USER $del_script $temp_del_file_name prg_exit_state 120]
   if {$prg_exit_state != 0} {
      ts_log_severe "deleting temp files failed $prg_exit_state:\n$result"
   } else {
      # if deletion worked we can remove the temp file
      # otherwise keep it, the next call to this function will then re-try to delete the files
      file delete $temp_del_file_name
   }
}
