...
proc compile_source_aimk {do_only_hooks compile_hosts report_var {compile_only 0}} {
   ...
   # Do not clear version info if the build failed - this might lead to get_version_info
   # waiting for the qconf binary - which does no longer exist.
   if {$error_count == 0} {
...
proc compile_source_cmake_make_build_dir {compile_hosts report_var {build_3rdparty_hosts_var ""}} {
   ...
   foreach host $compile_hosts {
      ...
      #
      # As CHECK_USER, like every other directory below the local spool dir
      # (cleanup_spool_dir_for_host() creates <spooldir>/<port> and the execd
      # spool the same way). root is not needed: only <spooldir> itself lives
      # in a root-owned parent, and it has to exist before any of this runs.
